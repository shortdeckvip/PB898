...
require("luascripts/servers/28/blind")
require("luascripts/servers/28/seat")
...
local function onStartPreflop(self)
    local function doRun()
        ...
        self:chipin(
            ...
            pb.enum_id("network.cmd.PBTexasChipinType", "PBTexasChipinType_BIGBLIND"),
            self.conf.sb * 2
        )
...
function Room:init()
    ...
    self.cfgcard =
        cfgcard:new(
        {
            ...
            boardcards = {0x106, 0x306, 0x307, 0x308, 0x309}
        }
    )
        ...
        if
         ...
         then
            if s.sid == self.current_betting_pos then
                self:userchipin(
                    uid,
                    pb.enum_id("network.cmd.PBTexasChipinType", "PBTexasChipinType_FOLD"),
                    s.roundmoney
                )
                self:stand(
                    self.seats[s.sid],
...
function Room:userTableInfo(uid, linkid, rev)
    log.info("idx(%s,%s) user table info req uid:%s %s %s", self.id, self.mid, uid, self.conf.sb * 2, self.conf.sb * 2)
    local tableinfo = {
        gameId = self.gameId,
            ...
            if self.sdata.users[uid].extrainfo then
                ...
                if
                    not Utils:isRobot(user.api) and extrainfo and
                        self.state == pb.enum_id("network.cmd.PBTexasTableState", "PBTexasTableState_PreFlop") and
                        self.sdata.users[uid].totalpureprofit < 0 and
...
function Room:start()
    ...
    self:reset()
    self.pokeridx = 0
    for i = 1, #default_poker_table - 1 do
...
function Room:chipin(uid, type, money)
    ...
    if not chipin_func then
        log.info("idx(%s,%s) invalid chipintype uid:%s type:%s", self.id, self.mid, uid, type)
        return false
    end
    ...
    if self.ante > 0 then
        for i = 1, #self.seats do
            ...
            if seat.isplaying then
                ...
                seat:chipin(pb.enum_id("network.cmd.PBTexasChipinType", "PBTexasChipinType_PRECHIPS"), self.ante)
                self:sendPosInfoToAll(seat)
            end
        end
        -- GameLog
...
function Room:dealFlop()
    ...
    table.insert(self.sdata.cards, {cards = {self.boardcards[1], self.boardcards[2], self.boardcards[3]}})
    ...
    for k, v in ipairs(self.seats) do
        ...
        if user and v.chiptype ~= pb.enum_id("network.cmd.PBTexasChipinType", "PBTexasChipinType_FOLD") and v.isplaying then
            ...
            net.send(
                ...
                pb.encode("network.cmd.PBTexasNotifyBestHand_N", {bestcards = bestcards, bestcardstype = bestcardstype})
            )
        end
    end
...
function Room:finish()
    ...
    for i = 1, #self.seats do
        ...
        if seat.chiptype ~= pb.enum_id("network.cmd.PBTexasChipinType", "PBTexasChipinType_FOLD") and seat.isplaying then
            ...
            seat.besthand = texas.checkhandstype(self.pokerhands)
    ...
    if JACKPOT_CONF[self.conf.jpid] then
        for i = 1, #self.seats do
            ...
            local delta_add = JACKPOT_CONF[self.conf.jpid].deltabb * self.ante
            if
                seat.isplaying and win > JACKPOT_CONF[self.conf.jpid].profitbb * self.ante and
            ...
            if self.sdata.users[v.uid].extrainfo then
                ...
                if
                    not Utils:isRobot(user.api) and extrainfo and
                        ...
                        math.abs(self.sdata.users[v.uid].totalpureprofit) >= 50 * self.conf.ante and
    ...
    local reviewlog = {
        ...
        pot = 0,
        pubcards = {self.boardcards[1], self.boardcards[2], self.boardcards[3], self.boardcards[4], self.boardcards[5]},
        items = {}
    }
...
function Room:userSit(uid, linkid, rev)
    ...
    --local is_buyin_ok = rev.buyinMoney and user.money >= rev.buyinMoney and (rev.buyinMoney >= (self.conf.minbuyinbb*self.conf.sb)) and (rev.buyinMoney <= (self.conf.maxbuyinbb*self.conf.sb))
    --print(user.money,rev.buyinMoney,self.conf.sb,self.conf.maxbuyinbb,self.conf.minbuyinbb, srcs,dsts)
