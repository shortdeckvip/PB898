...
function Reservation:init(t, s)
    self.table = t
    self.seat = s
    self.chipin_timeout_count = 0  -- 超时次数
    self.chipin_timeout_round = 0  -- 超时轮数
    self.state = pb.enum_id("network.cmd.PBTexasLeaveToSitState", "PBTexasLeaveToSitState_Cancel")
    self.is_standup = false  -- 是否站起
	self.reservation_timer = timer.create()  -- 留座定时器
...
function Reservation:chipinTimeoutCount()
    ...
    if self.state == pb.enum_id("network.cmd.PBTexasLeaveToSitState", "PBTexasLeaveToSitState_Cancel") then
        self.chipin_timeout_count = self.chipin_timeout_count + 1  -- 累加超时次数
...
function Reservation:checkSitResultSuccInTime()
    if self.state == pb.enum_id("network.cmd.PBTexasLeaveToSitState", "PBTexasLeaveToSitState_Reserve") then -- 预留留座离桌成功
