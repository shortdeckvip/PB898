...
local function onStartPreflop(self)
    local function doRun()
        ...
        self:intoNextState(pb.enum_id("network.cmd.PBSeotdaTableState", "PBSeotdaTableState_Betting1")) -- 第一轮下注
    end
...
function Room:userchipin(uid, type, money, client)
    ...
    if self:checkRoundOver() then
        ...
        elseif self.state == pb.enum_id("network.cmd.PBSeotdaTableState", "PBSeotdaTableState_Betting2") then  -- 若第二轮下注完还未结束，则需重赛发牌
        ...
        for sid, seat in ipairs(self.seats) do
            if seat and seat.isplaying and
                ...
                if seat.uid then
                    self.sdata.users = self.sdata.users or {}
                    self.sdata.users[seat.uid] = self.sdata.users[seat.uid] or {}
                    self.sdata.users[seat.uid].cards = { seat.handcards[1] }
                end
            end
        end
