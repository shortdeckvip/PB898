...
function Statistic:appendLogs(data, logid)
    ...
    local gameuserlog = {
        gamelog = {
            ...
            totalbet =  math.floor(data.totalbet or 0),
            totalprofit =  math.floor(data.totalprofit or 0),
            areas = data.areas,
    ...
    for k, v in pairs(data.users or {}) do
        table.insert(
            gameuserlog.userlog,
            {
                ...
                --ip				= v.ip,
                --api				= v.api,
                totalbet = math.floor(v.totalbet or 0),
                totalprofit = math.floor(v.totalprofit or 0),
                totalpureprofit = math.floor(v.totalpureprofit or 0),
                totalfee =  math.floor(v.totalfee or 0),
                --profits			= v.profits,
