...
local function parseIntoGameRoomReq(uid, linkid, msg)
    ...
    log.debug("PBIntoGameRoomReq uid:%s,%s,%s,%s", uid, rev.gameid, rev.matchid, tostring(rev.roomid))
end
...
local function parseTexasTableInfoReq(uid, linkid, msg)
    ...
    local r = MatchMgr:getRoomById(rev.idx.matchid, rev.idx.roomid)
    --print("parseQiuQiuTableInfoReq", rev.idx.matchid, rev.idx.roomid)
    if r == nil then
        log.error("no room:%s,%s", uid, rev.idx.roomid)
        return
    end
...
local function parseQiuQiuCardSaveReqResp(uid, linkid, msg)
    local rev = pb.decode("network.cmd.PBQiuQiuCardSaveReqResp", msg)
    if not rev or not rev.idx then
        log.debug("PBQiuQiuCardSaveReqResp %s,%s not valid msg pass", uid, linkid)
        return
    end
    local r = MatchMgr:getRoomById(rev.idx.matchid, rev.idx.roomid)
    if not r then
    ...
    r:userCardSaveReqResp(uid, linkid, rev)
end

local function parseQiuQiuCardConfirmReqResp(uid, linkid, msg)
    local rev = pb.decode("network.cmd.PBQiuQiuConfirmReqResp", msg)
    if not rev or not rev.idx then
        log.debug("parseQiuQiuCardConfirmReqResp %s,%s not valid msg pass", uid, linkid)
        return
    end
    local r = MatchMgr:getRoomById(rev.idx.matchid, rev.idx.roomid)
    ...
    r:userConfirmReqResp(uid, linkid, rev)
end
