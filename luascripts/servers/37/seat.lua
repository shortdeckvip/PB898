...
function Seat:init(t, sid)
    ...
    self.chiptype = pb.enum_id("network.cmd.PBQiuQiuChipinType", "PBQiuQiuChipinType_NULL") -- 下注操作类型
...
function Seat:reset()
    self.handtype = 0
    self.handcards = {-1, -1, -1, -1}
    self.roundchipintypes = {0, 0, 0, 0}
    self.roundchipinmoneys = {0, 0, 0, 0}
    self.money = 0
    self.roundmoney = 0
    self.chipinnum = 0
    self.chiptype = pb.enum_id("network.cmd.PBQiuQiuChipinType", "PBQiuQiuChipinType_NULL")
    self.lastchiptype = self.chiptype
    self.isplaying = false
    self.reraise = false
    self.bettingtime = 0
...
function Seat:chipin(type, money)
    ...
    self.chiptype = type
    self.chipinnum = self.roundmoney  -- 保存上一轮操作的筹码数 
    self.roundmoney = money  -- 本轮操作的筹码数 

    self.addon_time = 0
    self.total_time = self.table.bettingtime

    if money >= self.chips and type ~= pb.enum_id("network.cmd.PBQiuQiuChipinType", "PBQiuQiuChipinType_FOLD") then
        self.chiptype = pb.enum_id("network.cmd.PBQiuQiuChipinType", "PBQiuQiuChipinType_ALL_IN")  -- 全押 
        self.roundmoney = self.chips  -- 本轮操作的筹码数 
        money = self.chips  -- 本轮操作的筹码数 
    end

    -- 留座
    if type ~= pb.enum_id("network.cmd.PBQiuQiuChipinType", "PBQiuQiuChipinType_FOLD") then
        self.rv:resetBySys()
    end

    -- 每一轮操作记录
    if self.table.state > pb.enum_id("network.cmd.PBQiuQiuTableState", "PBQiuQiuTableState_Start") then
        local round = self.table.state - pb.enum_id("network.cmd.PBQiuQiuTableState", "PBQiuQiuTableState_PreChips") -- 经过了几轮操作
        self.roundchipintypes[round] = self.chiptype    -- 该轮操作类型
        self.roundchipinmoneys[round] = self.roundmoney -- 该轮操作金额 
    end
    ...
    if
        self.table.state == pb.enum_id("network.cmd.PBQiuQiuTableState", "PBQiuQiuTableState_None") or
            self.table.state == pb.enum_id("network.cmd.PBQiuQiuTableState", "PBQiuQiuTableState_PreChips") or
            self.table.state == pb.enum_id("network.cmd.PBQiuQiuTableState", "PBQiuQiuTableState_PreFlop")
     then
    ...
    elseif self.table.state == pb.enum_id("network.cmd.PBQiuQiuTableState", "PBQiuQiuTableState_River") then
    ...
    if self.table.state < pb.enum_id("network.cmd.PBQiuQiuTableState", "PBQiuQiuTableState_River") then --翻牌前
        if
            pb.enum_id("network.cmd.PBQiuQiuChipinType", "PBQiuQiuChipinType_FOLD") == self.chiptype and
                (pb.enum_id("network.cmd.PBQiuQiuChipinType", "PBQiuQiuChipinType_NULL") == self.lastchiptype or
                    pb.enum_id("network.cmd.PBQiuQiuChipinType", "PBQiuQiuChipinType_PRECHIPS") == self.lastchiptype)
         then
            self.table.sdata.users[self.uid].ugameinfo.texas.incpreflopfoldhands = 1
        end
        if
            pb.enum_id("network.cmd.PBQiuQiuChipinType", "PBQiuQiuChipinType_RAISE") == self.chiptype or
                pb.enum_id("network.cmd.PBQiuQiuChipinType", "PBQiuQiuChipinType_ALL_IN") == self.chiptype
         then
            self.table.sdata.users[self.uid].ugameinfo.texas.incpreflopraisehands = 1
        end
        if pb.enum_id("network.cmd.PBQiuQiuChipinType", "PBQiuQiuChipinType_CHECK") == self.chiptype then
