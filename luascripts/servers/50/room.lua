    ...
    local function doRun()
        ...
        if self.needJackPotResult then
            ...
            if not self.users[self.uid].isdebiting and self.freeSpinTimes <= 0 and not self.needJackPotResult then -- 如果未中免费旋转且未中jackpot
                Utils:credit(self, pb.enum_id("network.inter.MONEY_CHANGE_REASON", "MONEY_CHANGE_SLOTSEA_SETTLE")) -- 增加金额(输赢都更新金额)
                ...
                self.winMoney = 0
                self.sdata.jp = {}
                Utils:serializeMiniGame(SLOT_SEA_INFO, nil, global.stype())
            end
        end
    end
...
function Room:init()
    ...
    -- self.total_bets = SLOT_SEA_INFO.total_bets -- {} -- 存放各天的总下注额
    -- self.total_profit = SLOT_SEA_INFO.total_profit -- {} -- 存放各天的总收益
end
...
function Room:userInto(uid, linkid, rev)
    ...
    local t = {
        ...
        data = {
            ...
            allLineInfo = g.copy(SLOT_CONF.lineInfoSea) -- 线条信息
        } --data
    } --t
    ...
    if self.freeSpinTimes > 0 then -- 如果当前是免费旋转
        ...
        if not self:conf().isib then -- 如果不是indibet版本
            ...
            Utils:walletRpc(
                ...
                user.ip,
                -1 * self.bet,
                pb.enum_id("network.inter.MONEY_CHANGE_REASON", "MONEY_CHANGE_SLOTSEA_BET"),
                linkid,
            ...
            -- 减少身上金额
            Utils:debit(self, pb.enum_id("network.inter.MONEY_CHANGE_REASON", "MONEY_CHANGE_SLOTSEA_BET"))
...
function Room:userWalletResp(rev)
    ...
    for _, v in ipairs(rev.data) do
        ...
        if v.code >= 0 then
            ...
            -- Utils:debitRepay(
            --     self,
            --     pb.enum_id("network.inter.MONEY_CHANGE_REASON", "MONEY_CHANGE_SLOTSEA_SETTLE"),
            --     v,
            --     user,
            --     EnumRoomState.Show
            -- )
