...
function TeemPatti:getJokerCard(handcards)
    ...
    if cardValue(handcards[1]) == cardValue(handcards[2]) then
        local color = (cardColor(handcards[1]) % 4) +1
        if color == cardColor(handcards[2]) then
            color = (cardColor(handcards[2]) % 4) +1
        end
        jokerCard = cardValue(handcards[1]) + (color << 8)   -- 合成三条
    else
        if cardColor(handcards[1]) == cardColor(handcards[2]) then -- 如果手牌花色相同 
            ...
            if jokerCard == 0 then   -- 如果未能合成同花顺 
                --合成最大的同花 
                if (0xE ~= cardValue(handcards[1])) and (0xE ~= cardValue(handcards[2])) then
                    jokerCard = (handcards[1] & 0xFF00) + 0xE   -- A
                elseif (0xD ~= cardValue(handcards[1])) and (0xD ~= cardValue(handcards[2])) then
                    jokerCard = (handcards[1] & 0xFF00) + 0xD    -- K
                else   -- 下面这种情况不可能出现 
                    jokerCard = (handcards[1] & 0xFF00) + 0xC    -- Q
                end
            end     
            return jokerCard       
        end
