-- serverdev\luascripts\servers\53\usermsg.lua
...
local function parseBetReq(uid, linkid, msg)
    local rev = pb.decode("network.cmd.PBPveBetReq_C", msg) -- 解析消息包
    if not rev or not rev.idx then
...
local function parseHistoryReq(uid, linkid, msg)
    local rev = pb.decode("network.cmd.PBPveHistoryReq_C", msg)
    if not rev or not rev.idx then
        log.debug("network.cmd.PBPveHistoryReq_C uid=%s,linkid=%s not valid msg pass", uid, linkid)
        return
    end
    local r = MatchMgr:getRoomById(rev.idx.matchid, rev.idx.roomid)
    if r == nil then -- 房间不存在
...
Register(
    pb.enum_id("network.cmd.PBMainCmdID", "PBMainCmdID_Game"),
    pb.enum_id("network.cmd.PBGameSubCmdID", "PBGameSubCmdID_PveBetReq"),
    "",
    parseBetReq
)
Register(
    pb.enum_id("network.cmd.PBMainCmdID", "PBMainCmdID_Game"),
    pb.enum_id("network.cmd.PBGameSubCmdID", "PBGameSubCmdID_PveHistoryReq"),
    "",
    parseHistoryReq
)
Register(
