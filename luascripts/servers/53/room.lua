...
require(string.format("luascripts/servers/%d/teenpatti", global.stype()))
...
function Room:init()
    ...
    self.poker = TeenPatti:new() -- 每个房间一副牌
            ...
            user.co = coroutine.create(-- 创建一个新协程。
                function(user) -- 函数参数由resume()中的第2个实参传递
                    ...
                    elseif self.state == EnumRoomState.Show then -- 开牌阶段
                        ...
                        t.data.cardsTypeA = TeenPatti:GetCardsType(self.cardsA)
                        t.data.cardsTypeB = TeenPatti:GetCardsType(self.cardsB)
                    ...
                    t.data.player = user.playerinfo  or {} -- 玩家信息(玩家ID、昵称、金额等)
...
function Room:userBet(uid, linkid, rev)
    ...
    if not self:conf().isib and linkid then
        Utils:walletRpc(
            ...
            user.ip,
            -user_totalbet,
            pb.enum_id("network.inter.MONEY_CHANGE_REASON", "MONEY_CHANGE_TP2CARDS_BET"),
            linkid,
            self:conf().roomtype,
            self.id,
            self.mid,
...
function Room:calBetST()
    ...
    for _, v in ipairs(self.logmgr:getLogs()) do -- 遍历所有输赢日志(记录各局的赢方)
        ...
        if v.winpokertype then
            if v.winpokertype == 7 then  -- 3张A
                self.betst[v.winpokertype + 1].hitcount = self.betst[v.winpokertype + 1].hitcount + 1
            else
                self.betst[v.winpokertype + 2].hitcount = self.betst[v.winpokertype + 2].hitcount + 1
            end
        end
    end
end
...
function Room:show()
    ...
    if self:conf().isib then
        Utils:debit(self, pb.enum_id("network.inter.MONEY_CHANGE_REASON", "MONEY_CHANGE_TP2CARDS_BET"))
    ...
    self.cardsA = self.poker:getNCard(2) -- 获取2张牌
    self.cardsB = self.poker:getNCard(2) -- 获取2张牌
            ...
            if profit_rate < self:conf().profitrate_threshold_lowerlimit then
                ...
                if profit_rate < self:conf().profitrate_threshold_minilimit or rnd <= 5000 then
                    ...
                    if realPlayerWin > self:conf().profit_max_win then
                        ...
                        profit_rate, usertotalbet_inhand, usertotalprofit_inhand = self:getTotalProfitRate(winArea)
                    end
                end
            end
        ...
        if self:conf().single_profit_switch then -- 单人控制
            ...
            self.result_co = coroutine.create(
                function()
                    ...
                    self.cardsA = self.poker:getMaxCards(self.cardsA)
                    self.cardsB = self.poker:getMaxCards(self.cardsB)
                    -- 填写返回数据
                    local t = {
                        cardsNum = self.cardsNum, -- 本局发出的牌总张数
                        cardsA = { self.cardsA[1], self.cardsA[2], self.cardsA[3] }, -- A组牌数据
                        cardsB = { self.cardsB[1], self.cardsB[2], self.cardsB[3] }, -- B组牌数据
                        areainfo = {},
                        cardsTypeA = TeenPatti:GetCardsType(self.cardsA),
                        cardsTypeB = TeenPatti:GetCardsType(self.cardsB)
                    }
    ...
    if needSendResult then
        ...
        local t = {
            ...
            cardsA = { self.cardsA[1], self.cardsA[2], self.cardsA[3] }, -- A方牌数据
            cardsB = { self.cardsB[1], self.cardsB[2], self.cardsB[3] }, -- B方牌数据
            areainfo = {},
            cardsTypeA = TeenPatti:GetCardsType(self.cardsA),
            cardsTypeB = TeenPatti:GetCardsType(self.cardsB)
        }
...
function Room:finish()
    ...
    Utils:credit(self, pb.enum_id("network.inter.MONEY_CHANGE_REASON", "MONEY_CHANGE_TP2CARDS_SETTLE"))
...
function Room:kickout()
    if self.state ~= EnumRoomState.Finish then
        Utils:repay(self, pb.enum_id("network.inter.MONEY_CHANGE_REASON", "MONEY_CHANGE_TP2CARDS_SETTLE"))
    end
    for k, v in pairs(self.users) do -- 遍历该房间所有玩家
...
function Room:userWalletResp(rev)
    ...
    for _, v in ipairs(rev.data) do
        ...
        if v.code > 0 then
            ...
            Utils:debitRepay(
                self,
                pb.enum_id("network.inter.MONEY_CHANGE_REASON", "MONEY_CHANGE_TP2CARDS_SETTLE"),
                v,
                user,
                EnumRoomState.Show
            )
