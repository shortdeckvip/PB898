...
function Room:init()
    ...
    self.userbets = g.copy(DEFAULT_BET_TABLE) -- 当局所有玩家在每个下注区下注累计值
...
function Room:userBet(uid, linkid, rev)
    ...
    for k, v in ipairs(rev.data.areabet) do
        if v and v.bettype and type(v.bettype) == "number" and v.betvalue and type(v.betvalue) == "number" then
            if
                not g.isInTable(DEFAULT_BET_TYPE, v.bettype) or -- 下注类型非法
    ...
    if banker_money and banker_money > 0 then
        ...
        if
            betype_bets * (self:conf().betarea[EnumDragonTigerType.EnumDragonTigerType_Draw][1] - 1) >
        ...
        if
            betype_bets * (self:conf().betarea[EnumDragonTigerType.EnumDragonTigerType_Dragon][1] - 1) >
        ...
        if
            betype_bets * (self:conf().betarea[EnumDragonTigerType.EnumDragonTigerType_Tiger][1] - 1) >
                banker_money + self.bets[EnumDragonTigerType.EnumDragonTigerType_Draw] +
                    self.bets[EnumDragonTigerType.EnumDragonTigerType_Dragon]
         then
    ...
    if not ok then
        ...
        t.data.balance = self:getUserMoney(t.data.uid) - (user.totalbet or 0)
...
function Room:show()
    ...
    elseif self.bankmgr:banker() > 0 and robottotalbet > 0 then
        ...
        if profit_rate < self:conf().banker_profitrate_threshold_lowerlimit then
            ...
            profit_rate, usertotalbet_inhand, usertotalprofit_inhand = self:getRobotTotalProfitRate(wintype)
            if profit_rate < self:conf().banker_profitrate_threshold_minilimit and retrycnt < 2 then
                goto labelretry
            end
            if profit_rate < self:conf().banker_profitrate_threshold_minilimit and profit_rate < last_profitrate then
                dragon, tiger = tiger, dragon
                log.info("idx(%s,%s) greator banker swap cards is trigger", self.id, self.mid)
                wintype = self.poker:getWinType(dragon, tiger)
                profit_rate, usertotalbet_inhand, usertotalprofit_inhand = self:getRobotTotalProfitRate(wintype)
            end
        end
        ...
        if (not v.isdebiting and v.totalbet and v.totalbet > 0) or self.bankmgr:banker() == k then
            ...
            for _, bettype in ipairs(DEFAULT_BET_TYPE) do
                ...
                if bets > 0 then
                    ...
                    if
                        bettype == EnumDragonTigerType.EnumDragonTigerType_Dragon or
