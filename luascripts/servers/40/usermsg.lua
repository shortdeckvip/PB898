...
local function parseLeaveGameRoomReq(uid, linkid, msg)
    ...
    if r then
        ...
        --TODO: test debug
        r:userLeave(uid, linkid, rev.opcode, true)
    else
        net.send(
            linkid,
            uid,
...
local function parseChipinReq(uid, linkid, msg)
    local rev = pb.decode("network.cmd.PBDummyChipinReq", msg)
    if not rev or not rev.idx then
    ...
    r:userchipin(
        ...
        {
            tosid = rev.tosid,
            tozoneid = rev.tozoneid,
            foldcards = rev.foldcards,
            handcards = rev.handcards,
            context = rev.context,
            knock = rev.knock
        },
        true
...
local function parseDummyTableInfoReq(uid, linkid, msg)
    ...
    local r = MatchMgr:getRoomById(rev.idx.matchid, rev.idx.roomid)
    --print("parseDummyTableInfoReq", rev.idx.matchid, rev.idx.roomid)
    if r == nil then
        log.error("no room:%s,%s", uid, rev.idx.roomid)
        return
    end
...
local function parseChangeGameRoomReq(uid, linkid, msg)
    ...
    local rto = rm:getDummyRoom(r.id, Utils:isRobot(uid), ip)
    if rto then
        rto.islock = true
        r:userLeave(uid, 0)
        rto.islock = false
...
local function parseDummyReviewReq(uid, linkid, msg)
...
local function parseDummyPreOperateReq(uid, linkid, msg)
...
local function parseDummyTableListInfoReq(uid, linkid, msg)
    local rev = pb.decode("network.cmd.PBTexasTableListInfoReq", msg)
    if not rev or not rev.matchid or not rev.roomid then
        log.debug("parseDummyTableListInfoReq %s,%s not valid msg pass", uid, linkid)
        return
    end
...
local function parseDummyAddTimeReq(uid, linkid, msg)
...
Register(
    ...
    "",
    parseDummyTableInfoReq
)
Register(
    pb.enum_id("network.cmd.PBMainCmdID", "PBMainCmdID_Game"),
    pb.enum_id("network.cmd.PBGameSubCmdID", "PBGameSubCmdID_DummyChipinReq"),
    "",
    parseChipinReq
)
Register(
