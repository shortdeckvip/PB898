...
-- 
local function parseShowOneCardReq(uid, linkid, msg)
    local rev = pb.decode("network.cmd.PBSeotdaShowOneCardReq", msg)
    if not rev or not rev.idx or not rev.idx.matchid or not rev.idx.roomid then
        log.debug("parseShowOneCardReq() uid=%s,linkid=%s not valid msg pass", uid, linkid)
        return
    end
    local r = MatchMgr:getRoomById(rev.idx.matchid, rev.idx.roomid)
    if not r then
        log.error("parseShowOneCardReq() no room. uid=%s, roomid=%s", uid, rev.idx.roomid)
        return
    end
    r:userShowOneCardReq(uid, linkid, rev)
end



-- 
local function parseCompareCardsReq(uid, linkid, msg)
    local rev = pb.decode("network.cmd.PBSeotdaCompareCardsReq", msg)
    if not rev or not rev.idx or not rev.idx.matchid or not rev.idx.roomid then
        log.debug("parseCompareCardsReq() uid=%s,linkid=%s not valid msg pass", uid, linkid)
        return
    end
    local r = MatchMgr:getRoomById(rev.idx.matchid, rev.idx.roomid)
    if not r then
        log.error("parseCompareCardsReq() no room. uid=%s, roomid=%s", uid, rev.idx.roomid)
        return
    end
    r:userCompareCardsReq(uid, linkid, rev)
end
...
Register(
    pb.enum_id("network.cmd.PBMainCmdID", "PBMainCmdID_Game"),
    pb.enum_id("network.cmd.PBGameSubCmdID", "PBGameSubCmdID_SeotdaShowOneCardReq"),
    "",
    parseShowOneCardReq
)
...
Register(
    pb.enum_id("network.cmd.PBMainCmdID", "PBMainCmdID_Game"),
    pb.enum_id("network.cmd.PBGameSubCmdID", "PBGameSubCmdID_SeotdaCompareCardsReq"),  -- 选择要比较的牌
    "",
    parseCompareCardsReq
)
