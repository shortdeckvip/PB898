-- serverdev\luascripts\servers\35\usermsg.lua
...
local function parseIntoGameRoomReq(uid, linkid, msg)
	...
	if not r then
		r = rm:getAvaiableRoom()  -- 获取一个人数最多的房间  
	end
...
local function parseBetReq(uid, linkid, msg)
	local rev = pb.decode("network.cmd.PBAndarBaharBetReq_C", msg)  -- 解析消息包 
	if not rev or not rev.idx then
...
local function parseHistoryReq(uid, linkid, msg)
	local rev = pb.decode("network.cmd.PBAndarBaharHistoryReq_C", msg)
	if not rev or not rev.idx then
		log.debug("network.cmd.PBAndarBaharHistoryReq_C %s,%s not valid msg pass", uid, linkid)
		return
	end
	local r = MatchMgr:getRoomById(rev.idx.matchid, rev.idx.roomid)
	if r == nil then -- 房间不存在 
...
local function parseOnlineListReq(uid, linkid, msg)
	local rev = pb.decode("network.cmd.PBAndarBaharOnlineListReq_C", msg)
	if not rev or not rev.idx then
		log.debug("network.cmd.PBAndarBaharOnlineListReq_C %s,%s not valid msg pass", uid, linkid)
		return
	end
	local r = MatchMgr:getRoomById(rev.idx.matchid, rev.idx.roomid)
	if r == nil then
...
Register(pb.enum_id("network.cmd.PBMainCmdID", "PBMainCmdID_Game"), pb.enum_id("network.cmd.PBGameSubCmdID", "PBGameSubCmdID_AndarBaharBetReq"), "", parseBetReq)
Register(pb.enum_id("network.cmd.PBMainCmdID", "PBMainCmdID_Game"), pb.enum_id("network.cmd.PBGameSubCmdID", "PBGameSubCmdID_AndarBaharHistoryReq"), "", parseHistoryReq)
Register(pb.enum_id("network.cmd.PBMainCmdID", "PBMainCmdID_Game"), pb.enum_id("network.cmd.PBGameSubCmdID", "PBGameSubCmdID_AndarBaharOnlineListReq"), "", parseOnlineListReq)
