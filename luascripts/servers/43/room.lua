...
local TimerID = {
    ...
    TimerID_FreeSpinTimes = {3, 3 * 1000, 0} --id, interval(ms), timestamp(ms)
}
...
local function GetLineData(lineNo, cards, slotConf)
    local lineData = {}
    lineData[1] = cards[(slotConf.lineInfo[lineNo].col[1] - 1) * slotConf.lineCol + 1]
    lineData[2] = cards[(slotConf.lineInfo[lineNo].col[2] - 1) * slotConf.lineCol + 2]
    lineData[3] = cards[(slotConf.lineInfo[lineNo].col[3] - 1) * slotConf.lineCol + 3]
    lineData[4] = cards[(slotConf.lineInfo[lineNo].col[4] - 1) * slotConf.lineCol + 4]
    lineData[5] = cards[(slotConf.lineInfo[lineNo].col[5] - 1) * slotConf.lineCol + 5]
...
-- 返回值: 返回本局所有线条赢得的倍数
local function GetAllLineWinTimes(cards, slotConf, type)
    local lineCards = {}
    local winLineTimes = 0
    for i = 1, slotConf.lineNum do -- 遍历每条线
    ...
    local function doRun()
        ...
        for uid, user in pairs(self.users) do
            ...
            if user.state == EnumUserState.Logout and global.ctsec() >= user.logoutts + MYCONF.logout_to_kickout_secs then
                log.info(
                    "idx(%s,%s,%s) onCheck user logouttime=%s, currentTime=%s",
                    self.id,
                    self.mid,
                    tostring(self.logid),
                    user.logoutts,
...
function Room:init()
    ...
    log.debug(
        "init() profitrate_threshold_maxdays=%s,profitrate_threshold_minilimit=%s,profitrate_threshold_lowerlimit=%s",
        self.conf.profitrate_threshold_maxdays,
...
function Room:check()
    ...
    log.info("idx(%s,%s,%s) Room:check()", self.id, self.mid, tostring(self.logid))
    ...
    if self.uid and self.uid ~= 0 then -- 如有该桌已有玩家
        if self.uid ~= uid then
            ...
            if self.users[uid] then
                log.debug("idx(%s,%s) uid=%s dqw112", self.id, self.mid, uid)
...
function Room:userLeave(uid, linkid)
    ...
    log.info("idx(%s,%s,%s) user leave uid=%s", self.id, self.mid, tostring(self.logid), uid)
    ...
    log.info("idx(%s,%s,%s) userLeave:%s,%s,%s", self.id, self.mid, tostring(self.logid), uid, user.gamecount or 0, 0)
            ...
            if lineTimes > 0 then
                table.insert(
                    t.winLines,
                    {
                        ...
                        picNum = num,
                        winTimes = lineTimes,
                        winMoney = lineTimes * self.bet / SLOT_CONF.lineNum
                    }
                )
            end
        end
...
function Room:userSlotConfInfo(uid, linkid, rev)
    ...
    log.info("idx(%s,%s) uid=%s, PBSlotConfResp_S: %s", self.id, self.mid, uid, cjson.encode(t))
end
