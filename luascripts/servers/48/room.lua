...
local EnumPveType = {
    EnumPveType_BetArea1 = 1, --下注区1
    EnumPveType_BetArea2 = 2, --下注区2
    EnumPveType_BetArea3 = 3, --下注区3
    EnumPveType_BetArea4 = 4, --下注区4
    EnumPveType_BetArea5 = 5, --下注区5
    EnumPveType_BetArea6 = 6, --下注区6
}
...
function Room:userInto(uid, linkid, rev)
    ...
    local t = {
        ...
        data = {
            ...
            odds = { 3, 5, 10, 20, 100 }, -- 下注区域设置(赔率, limit-min, limit-max)
            ...
            user.co = coroutine.create(-- 创建一个新协程。
                function(user) -- 函数参数由resume()中的第2个实参传递
                    ...
                    elseif self.state == EnumRoomState.Show then -- 开牌阶段
                        ...
                        t.data.cardsA = g.copy(self.cards)
...
function Room:userBet(uid, linkid, rev)
    ...
    if not self:conf().isib and linkid then
        Utils:walletRpc(
            ...
            user.ip,
            -user_totalbet,
            pb.enum_id("network.inter.MONEY_CHANGE_REASON", "MONEY_CHANGE_DICE6_BET"),
            linkid,
            self:conf().roomtype,
            self.id,
            self.mid,
...
local function onCheck(self)
    local function doRun()
        ...
        self.stateOnce = true -- 确保该状态下只执行一次
...
local function onCreateRobot(self)
    local function doRun()
        ...
        elseif self.state == EnumRoomState.Start then -- 如果是开始状态
            ...
            if currentTimeMS - self.stateBeginTime >= TimerID.TimerID_Start[2] then -- 如果超出开始阶段时长
                self:betting() -- 开始下注
            end
...
function Room:show()
    ...
    if self:conf().isib then
        Utils:debit(self, pb.enum_id("network.inter.MONEY_CHANGE_REASON", "MONEY_CHANGE_DICE6_BET"))
    ...
    self.cards = self:getCardsRand() or {} -- 随机获取一组牌
    winArea = self:getWinType(self.cards)
    ...
    if usertotalbet > 0 then
        if self:conf().global_profit_switch then -- 全局控制
            ...
            local profit_rate, usertotalbet_inhand, usertotalprofit_inhand = self:getTotalProfitRate(self.cards, winArea)
            ...
            if profit_rate < self:conf().profitrate_threshold_lowerlimit then
                ...
                if profit_rate < self:conf().profitrate_threshold_minilimit or rnd <= 5000 then
                    local realPlayerWin = self:GetRealPlayerWin(self.cards)
                    if realPlayerWin > self:conf().profit_max_win then
                        -- 需要确保系统赢，换牌系统不一定赢
                        self:getCardsByResult(-1, 0) -- 确保真实玩家输
                        winArea = self:getWinType(self.cards) -- 获取所有赢的区域
                    end
                end
            end
            ...
            self.result_co = coroutine.create(
                function()
                    ...
                    local t = {
                        ...
                        cardsA = g.copy(self.cards), -- A组牌数据
                        areainfo = {},
                    }
                    ...
                    for _, v in pairs(DEFAULT_BET_TYPE) do
                        table.insert(
                            ...
                            {
                                bettype = v,
                                iswin = self:isWinArea(self.cards, v)
                            }
                        )
                    end
                    ...
                    -- 生成一条记录
                    local logitem = { wintype = g.copy(winArea), winpokertype = nil, cards=g.copy(self.cards) } -- 存放哪一方赢
    ...
    if needSendResult then
        ...
        local t = {
            ...
            cardsA = g.copy(self.cards), -- A方牌数据
            areainfo = {}
        }
...
function Room:finish()
            ...
            for _, bettype in ipairs(DEFAULT_BET_TYPE) do -- 遍历所有下注区
                ...
                if bets > 0 then -- 如果该玩家在该下注区下注了
                    ...
                    if g.isInTable(lastlogitem.wintype, bettype) then -- 押中 判断当前区域是否赢
                        ...
                        profit = bets * self:getWinTimes(self.cards, bettype)
                        --扣除服务费 盈利下注区的5%
    ...
    Utils:credit(self, pb.enum_id("network.inter.MONEY_CHANGE_REASON", "MONEY_CHANGE_DICE6_SETTLE"))
...
function Room:kickout()
    if self.state ~= EnumRoomState.Finish then
        Utils:repay(self, pb.enum_id("network.inter.MONEY_CHANGE_REASON", "MONEY_CHANGE_DICE6_SETTLE"))
    end
    for k, v in pairs(self.users) do -- 遍历该房间所有玩家
...
-- 参数 cards: 牌数据
-- 参数 winArea： 所有赢的区域
function Room:getTotalProfitRate(cards, winArea)
    ...
    for _, v in pairs(EnumPveType) do
        ...
        if g.isInTable(winArea, v) then -- 如果该下注区赢了
            ...
            usertotalprofit_inhand = usertotalprofit_inhand + (self.userbets[v] or 0) * self:getWinTimes(cards, v)
...
function Room:userWalletResp(rev)
    ...
    for _, v in ipairs(rev.data) do
        ...
        if v.code > 0 then
            ...
            Utils:debitRepay(
                self,
                pb.enum_id("network.inter.MONEY_CHANGE_REASON", "MONEY_CHANGE_DICE6_SETTLE"),
                v,
                user,
                EnumRoomState.Show
            )
            ...
            if v ~= 0 then -- 如果有玩家在该下注区下注了
                table.insert(
                    t.data.betdata.areabet,
                    {
                        ...
                        areatotal = v, -- 该区域的总下注额
                        --odds			= self:conf() and self:conf().betarea and self:conf().betarea[k][1],
                        odds = self:getWinTimes(self.cards, k)
                    }
                )
            end
        end
...
-- 参数 cards: 牌数据
-- 返回值: 返回真实玩家该局总盈利额
function Room:GetRealPlayerWin(cards)
    local realPlayerWin = 0
    local userTotalBet = 0 -- 真实玩家下注总金额

    -- 根据牌数据计算各区域中的骰子个数
    local arrayNum = { 0, 0, 0, 0, 0, 0 }
    for k, v in ipairs(cards) do
        arrayNum[v] = arrayNum[v] + 1
    end
    ...
    for k, v in ipairs(DEFAULT_BET_TYPE) do
        if arrayNum[v] == 2 then
            realPlayerWin = realPlayerWin + self.userbets[v] * 3
        elseif arrayNum[v] == 3 then
            realPlayerWin = realPlayerWin + self.userbets[v] * 5
        elseif arrayNum[v] == 4 then
            realPlayerWin = realPlayerWin + self.userbets[v] * 10
        elseif arrayNum[v] == 5 then
            realPlayerWin = realPlayerWin + self.userbets[v] * 20
        elseif arrayNum[v] == 6 then
            realPlayerWin = realPlayerWin + self.userbets[v] * 100
        end
    end
...
function Room:getCardsByResult(res, maxwin)
    ...
    local cardsBuf -- 保存最优的牌数据
    local realPlayerMaxWin = 0x7FFFFFFF

    -- 循环发牌，寻找需要的牌型
    for i = 1, 100 do
        self.cards = self:getCardsRand() or {}
        local realPlayerWin = self:GetRealPlayerWin(self.cards) -- 该真实玩家在该牌局状态下赢取到的金额
        if i == 1 then
            cardsBuf = g.copy(self.cards)
        ...
        if res > 0 then -- 真实玩家赢
            if realPlayerWin > 0 and realPlayerWin <= maxwin then
                hasFind = true
                break
            elseif realPlayerWin == 0 then -- 真实玩家不输不赢
                realPlayerMaxWin = realPlayerWin
                cardsBuf = g.copy(self.cards)
            elseif realPlayerWin <= maxwin then
    ...
    if not hasFind then -- 如果未找出满足条件的牌
        self.cards = g.copy(cardsBuf)
    end
end

-- 随机发牌
function Room:getCardsRand()
    --self.cards = {}
    local cards = {}
    for i = 1, 6 do
        cards[i] = rand.rand_between(1, 6)
    end
    return cards
end

-- 根据牌数据判断某下注区是否赢
function Room:isWinArea(cards, betArea)
    local arrayNum = { 0, 0, 0, 0, 0, 0 }
    for k, v in pairs(cards) do
        arrayNum[v] = (arrayNum[v] or 0) + 1
    end
    if arrayNum[betArea] and arrayNum[betArea] > 1 then
        return true
    end
    return false
end
...
-- 获取某区域赢的倍数
function Room:getWinTimes(cards, betArea)
    local arrayNum = { 0, 0, 0, 0, 0, 0 }
    for k, v in pairs(cards) do
        arrayNum[v] = (arrayNum[v] or 0) + 1
    end
    if not arrayNum[betArea] or arrayNum[betArea] < 2 then
        return 0
    elseif arrayNum[betArea] == 2 then
        return 3
    elseif arrayNum[betArea] == 3 then
        return 5
    elseif arrayNum[betArea] == 4 then
        return 10
    elseif arrayNum[betArea] == 5 then
        return 20
    elseif arrayNum[betArea] == 6 then
        return 100
    end
    return 0
end
...
-- 获取所有赢的区域
function Room:getWinType(cards)
    local winArea = {}
    local arrayNum = { 0, 0, 0, 0, 0, 0 }
    for k, v in pairs(cards) do
        arrayNum[v] = (arrayNum[v] or 0) + 1
    end
    for i = 1, #arrayNum, 1 do
        if arrayNum[i] >= 2 then
            table.insert(winArea, i)
        end
    end
    return winArea
end
